...
from admin_kb import admin_kb_menu, admin_kb_log, admin_kb_users, admin_kb, not_superuser, admin_kb_btn_back, \
    ...
    admin_kb_btn_close, admin_kb_reserve, admin_kb_select_cancel_reserve
    ...
    match call.data:
        ...
        case 'cancellation_reserve':
            bot.edit_message_text(
                message_id=call.message.message_id,
                chat_id=call.from_user.id,
                text=f"Выберите столик для отмены брони.",
...
def calsel_reserve(call:types.CallbackQuery):
    ...
    con.cancel_reserve(table_number=call.data.split('*')[1])
    bot.edit_message_text(
        message_id=call.message.message_id,
        chat_id=call.from_user.id,
        text=f"Вы отменили бронь столика № {call.data.split('*')[1]}",
